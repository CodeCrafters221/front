...
// ------------------------------------------ ON INIT ------------------------------------------
  ...
  this.aiConfigs.systemInstructions = configs.SYSTEM_INSTRUCTIONS[0]?.parts[0].text
  this.aiConfigs.model = configs.MODEL_NAME
  this.aiConfigs.modelName = this.models.find(model => model.value == configs.MODEL_NAME).name
  this.aiConfigs.assistantName = configs.ASSISTANT_NAME
  this.aiConfigs.generationConfig = configs.GENERATION_CONFIG
